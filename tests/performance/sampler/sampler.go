...
func NewPodMetricsSampler(cl client.Client, datasetDescription string, pod types.NamespacedName) PodMetricsSampler {
	...
	sampler.SampleConsumption(fmt.Sprintf("Start of sample for dataset %s", datasetDescription))
	return sampler
}
...
func (s *podMetricsSampler) SampleConsumption(label string) error {
	...
	fmt.Printf("\nMETRICS (%v) (%s) CPU:%v Memory:%v\n", time.Now(), label, podMetrics.Containers[0].Usage.Cpu(), podMetrics.Containers[0].Usage.Memory())
	return nil
}
