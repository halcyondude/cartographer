...
import (
	...
	"github.com/vmware-tanzu/cartographer/tests/performance/sampler"
...
var _ = Describe("Benchmark", func() {
	...
	BeforeEach(func() {
		...
		s = sampler.NewPodMetricsSampler(c, fmt.Sprintf("%dscs_%dworkloads_%drunnables_%druns", numSupplyChains, numWorkloads, numRunnables, numRunnableRuns), GetCartographerControllerPod())

		testObjCrdBytes, err := os.ReadFile("../resources/crds/test.run_testobjs.yaml")
		Expect(err).NotTo(HaveOccurred())
		...
		It("eventually completes processing all workloads and runnables", func() {
			...
			Eventually(func(g Gomega) {
				...
				for nWorkload := 0; nWorkload < numWorkloads; nWorkload++ {
					...
					Expect(s.SampleConsumption(fmt.Sprintf("workload %d finished\n", nWorkload))).To(Succeed())
