...
for workloads in 2 500 1250 1500; do
  ...
  go run -modfile hack/tools/go.mod github.com/onsi/ginkgo/ginkgo -r tests/performance | tee bench_"$workloads"_workloads_w_runnables_1_run.txt
done
...
for workloads in 10 50 100 250; do
  ...
  export CARTO_BENCH_N_WORKLOADS="$workloads"
  export CARTO_BENCH_N_RUNNABLES="$workloads"
  go run -modfile hack/tools/go.mod github.com/onsi/ginkgo/ginkgo -r tests/performance | tee bench_"$workloads"_workloads_w_runnables_3_runs.txt
done
...
for runnables in 5 25 50; do
  for runs in 5 25 50 ; do
      ...
      go run -modfile hack/tools/go.mod github.com/onsi/ginkgo/ginkgo -r tests/performance | tee bench_"$runnables"_runnables_with_"$runs"_runs.txt
  done
done
